...
```{r trim_group0}
# trim off group 0 "don't bother" variables, which will not be further considered
beans <-
...
val_index <-
  createDataPartition(y = beans$Class,
                      ...
                      list = F)
# split the data
beans_train <- beans[-val_index,]
beans_val <- beans[val_index,]
```
...
beans_train %>%
  ...
  summarize(counts = n())
```
The number of rows ad up, and both sets have all 7 bean classes.

To facilitate the use of algorithms under caret's train function, the predictors and outcomes will be split

```{r x_y_split}
x_train <- subset(beans_train, select = -c(Class))
y_train <- subset(beans_train, select = c(Class))

x_val <- subset(beans_val, select = -c(Class))
y_val <- subset(beans_val, select = c(Class))
```
...
model_1a <-
  train(y_train ~ Perimeter + AspectRation, 
        method = 'glm',
        data = x_train)
```
