...
tenfold_xval <-
  trainControl(method = 'cv',
               number = 10,
               p = 0.9,
               summaryFunction = ) # having prSummary makes it throw an error about how it's not compatible with 7 Classes

# Will need custom F1 func...
```
...
model_2a <-
  train(y = beans_y_train$Class,
        ...
        metric = 'F1') # F1 metric for performance (keeps saying "not in result set"...)

# The metric = F1 isn't problematic; the summaryFunction arg under trainControl (which specifies the "performance metrics across resamples", according to the doc at datacamp) is
