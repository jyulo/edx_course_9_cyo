...
# Been told that this should take same args as a function called defaultSummary()
# data = a dataframe with columns 'obs' and 'pred' (actual and predicted)
...
```{r test_matrix}
test_table <- data.table(actual = c(1, 1, 2, 2, 2, 3, 4),
           predicted = c(1, 1, 2, 2, 1, 3, 4))

test_table
```

```{r table_test}
table(test_table$predicted, test_table$actual)
```
```{r f1_test}
f1(test_table$predicted, test_table$actual) # okay so the f1 function returns something... and the right answers to boot!
```
...
# 
# model_1a <-
#   train(beans_y_train$Class ~ Perimeter + AspectRation, 
#         method = 'glm',
#         data = beans_x_train)
```
...
nearest_neighbors <- seq(1, 100, 10)
