...
forest_grid <-
  expand.grid(
    mtry = 2:4, # how many variables to use for splitting at each tree?
    splitrule = 'gini', # for classification
    min.node.size = c(1:522) # how many instances minimum in a node? 552 is chosen since it's the smallest class in the full set
  )
...
model_4 <-
  train(y = beans_y_train$Class,
        ...
        num.trees = 500, # arbitrary; will change with literature search
        trControl = tenfold_xval, # use your 10-fold X-val 
        metric = 'mF1') # my custom metric
