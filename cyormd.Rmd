...
text(model_3d$finalModel, srt = 60, cex = 0.5)
```
...
model_4 <-
  train(y = beans_y_train$Class,
        ...
        num.trees = 400, # 1K is too much; new goal is to find out how FEW trees you can get away with and still have similar performance qualities
        trControl = tenfold_xval, # use your 10-fold X-val 
        metric = 'mF1') # my custom metric
...
levels(beans_y_val$Class) == levels(validation_preds$predictions) # check for level matching
# beans_y_val$Class # checking that format is right
```

```{r validation_LR}
validation_preds_LR <- predict(model_1c$finalModel, beans_x_val)

confusionMatrix(validation_preds_LR$predictions, beans_y_val$Class)
```

```{r validation_KNN}
validation_preds_KNN <- predict(model_2c$finalModel, beans_x_val)

confusionMatrix(validation_preds_KNN$predictions, beans_y_val$Class)
```

```{r validation_2_factor_DT}
validation_preds_2DT <- predict(model_3a$finalModel, beans_x_val)

confusionMatrix(validation_preds_2DT$predictions, beans_y_val$Class)
```

```{r validation_4_factor_DT}
validation_preds_4DT <- predict(model_3d$finalModel, beans_x_val)

confusionMatrix(validation_preds_4DT$predictions, beans_y_val$Class)
```
