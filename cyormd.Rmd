...
forest_grid <-
  expand.grid(
    ...
    min.node.size = 10:40 # from the big 1K run; peak region is 10-30, but 40 included just in case
  )
...
model_4 <-
  train(y = beans_y_train$Class,
        ...
        num.trees = 750, # try this one. If no better, try 600 then back to 500...
        trControl = tenfold_xval, # use your 10-fold X-val 
        metric = 'mF1') # my custom metric
...
# there is a diminishing return thing going on for sure; chill out with the ntrees... Try 750?
# also experiment with more min node sizes and even mtrys
```
```{r model_4_plot}
plot(model_4) # when holding mtry at 2 and expanding range of nodes to 1:50, you see that the peak region is 10-30
```
## Validation Run
```{r validation_run}
validation_preds <- predict(model_4$finalModel, beans_x_val)
beans_y_val$Class <- as.factor(beans_y_val$Class) # convert to factor class
beans_y_cal$Class
# confusionMatrix(validation_preds$predictions, beans_y_val)
```
